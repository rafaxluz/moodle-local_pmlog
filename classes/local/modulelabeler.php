 ...
 */
class modulelabeler {
    /** @var array<int, string|null> Cache of resolved labels by CMID. */
    private array $labelcache = [];

    /** @var array<string, string>|null Cache of module-name to label mappings. */
    private ?array $labelmap = null;
    ...
    public function label_from_cmid(?int $cmid): ?string {
        ...
        if (array_key_exists($cmid, $this->labelcache)) {
            return $this->labelcache[$cmid];
        }
        ...
        if (empty($modname)) {
            $this->labelcache[$cmid] = null;
            return null;
        }

        $modname = strtolower((string)$modname);
        $label = $this->get_label_map()[$modname] ?? get_string('activity_generic_open', 'local_pmlog', ucfirst($modname));
        $this->labelcache[$cmid] = $label;

        return $label;
    }

    /**
     * Get the static map of module names to labels.
     *
     * @return array<string, string>
     */
    private function get_label_map(): array {
